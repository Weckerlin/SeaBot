...
namespace SeaBotCore.BotMethods
{
    ...
    public static class Outposts
    {
        public static void ConfirmOutpost()
        {
            ...
            //var readyoutposts = Core.LocalPlayer.Outposts.Where(n => n.RequiredCrew <= n.Crew).ToList();
            //foreach (var outpost in readyoutposts)
            //{
            //    if (outpost.CargoOnTheWay == 0)
            //    {
            //        Core.LocalPlayer.Outposts.FirstOrDefault(n => n.DefId == outpost.DefId).Done = true;
            //        Networking.AddTask(new Task.ConfirmOutpostTask(outpost.DefId));

            //        // unlock all
            //        var outdef = Definitions.OutpostDef.Barrels.Barrel.Where(n => n.DefId == outpost.DefId)
            //            .FirstOrDefault();
